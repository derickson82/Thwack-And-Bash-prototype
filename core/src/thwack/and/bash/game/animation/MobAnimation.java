...
public class MobAnimation {
	...
	public void update(float delta, int newDir) {
		if (addingAnimations) {
			System.err
					.println("You need to call endAddingAnimations() in MOBAnimations class before you can update it");
		}
		if (newDir != -1) {
			dir = newDir;
			if (lastDir != dir) {
	...
	public void setAnimation(Animation animation, int id) {
		if (addingAnimations) {
			...
			System.err
					.println("You need to call startAddingAnimations() in MOBAnimations class before you can update it");
		}
